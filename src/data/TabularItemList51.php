...
/**
 * TabularItemList51 is the main class for parsing and validating tabular COUNTER R5.1 Report Item lists
 *
...
class TabularItemList51 extends JsonReportItemList51
{
    ...
    protected function parseDocument(): void
    {
        ...
        $isItemReport = $this->config->isItemReport($this->reportHeader->getReportId());
        $hasUnusableItems = false;
        $index = 0;
        foreach ($sheet->getRowIterator($headingsRowNumber + 1) as $position => $row) {
            $this->setIndex($index);
            $rowValues = $this->getRowValues($row);
            $item = new TabularItem51($this, $position, $index, $rowValues);
            $this->bodyRowsParsed++;
            if ($item->isUsable()) {
                if ($isItemReport) {
                    $parentItem = $item->getParentItem();
                    $parentHash = $parentItem->getHash();
                    $reportItem = $item->getReportItem();
                    $itemHash = $reportItem->getHash();
                    if (isset($this->items[$itemHash]) && $this->items[$itemHash] !== $parentHash) {
                        $message = 'Report_Item previously occured with a different Parent';
                        $data = $this->formatData('Item', $reportItem->get('Item')) . ' (first occurence in row ' .
                            $this->parents[$this->items[$itemHash]]->get('Items')
                                ->getItemForHash($itemHash)
                                ->getPosition() . ')';
                        $this->addCriticalError($message, $message, $position, $data);
                        $this->setUnusable();
                        $hasUnusableItems = true;
                    } else {
                        if (isset($this->parents[$parentHash])) {
                            $this->parents[$parentHash]->merge($parentItem);
                            if (! $parentItem->isUsable()) {
                                $this->setUnusable();
                                $hasUnusableItems = true;
                            }
                        } else {
                            $this->parents[$parentHash] = $parentItem;
                        }
                        $this->items[$itemHash] = $parentHash;
                    }
                } else {
                    $reportItem = $item->getReportItem();
                    $itemHash = $reportItem->getHash();
                    if (isset($this->items[$itemHash])) {
                        $this->items[$itemHash]->merge($reportItem);
                        if (! $reportItem->isUsable()) {
                            $this->setUnusable();
                            $hasUnusableItems = true;
                        }
                    } else {
                        $this->items[$itemHash] = $reportItem;
                    }
                }
                $this->addMetricTypesPresent($item->getMetricTypesPresent());
            } else {
    ...
    public function checkDataTypes(): void
    {
        if ($this->parents === null) {
            throw new \LogicException("TabularItemList51 already cleaned up");
        }

        if ($this->config->isItemReport($this->reportHeader->getReportId())) {
            foreach ($this->parents as $parent) {
                $parent->get('Items')->checkDataTypes();
            }
        } else {
            parent::checkDataTypes();
        }
    }
    ...
    public function checkMetricRelations(): void
    {
        ...
        if ($this->config->isItemReport($this->reportHeader->getReportId())) {
            foreach ($this->parents as $parent) {
                $parent->get('Items')->checkMetricRelations();
            }
        } else {
            parent::checkMetricRelations();
        }
    }
    ...
    {
        if ($this->parents === null) {
            throw new \LogicException("TabularItemList already cleaned up");
        }
